...
const StoryViewer = ({ viewStory, setViewStory }) => {
  ...
  useEffect(() => {
    ...
    if (viewStory && viewStory.media_type !== "video") {
      ...
      const setTime = 100; // update progress every 100ms,0.1 second
      let elapsed = 0; // time elapsed
        // update progress bar

      progressInterval = setInterval(() => {
        elapsed += setTime;
        setProgress((elapsed / duration) * 100); // calculate progress percentage (100/10000)*100 = 1
      }, setTime);
  ...
  // for displaying content when click on story card
  const renderContent = () => {
